...
function buildAcsiPayload(row: AcsiDbRow) {
  const dbKeyMap: Record<AcsiSubscaleKey, number> = {
    coping:        row.score_coping,
    peaking:       row.score_peaking,
    goalSetting:   row.score_goal_setting,
    concentration: row.score_concentration,
    freedom:       row.score_freedom,
    confidence:    row.score_confidence,
    coachability:  row.score_coachability,
  };

  const order: AcsiSubscaleKey[] = [
    "coping", "peaking", "goalSetting", "concentration", "freedom", "confidence", "coachability",
  ];

  const subscales = order.map((key) => {
    const score = dbKeyMap[key];
    let band: AcsiBand;
  ...
  return {
    report: { subscales, totalScore: row.total_score },
    respondent: {
...
function buildCsaiPayload(row: CsaiDbRow) {
  const dbKeyMap: Record<CsaiSubscaleKey, number> = {
    cognitive:  row.score_cognitive,
    somatic:    row.score_somatic,
    confidence: row.score_confidence,
  };
...
function buildDasPayload(row: DasDbRow) {
  const order: DasSubscaleKey[] = [
    "externalApproval", "lovability", "achievement", "perfectionism",
    "entitlement", "omnipotence", "externalControl",
  ];
  const scoreMap: Record<DasSubscaleKey, number> = {
    externalApproval: row.score_external_approval,
    lovability:       row.score_lovability,
    achievement:      row.score_achievement,
    perfectionism:    row.score_perfectionism,
    entitlement:      row.score_entitlement,
    omnipotence:      row.score_omnipotence,
    externalControl:  row.score_external_control,
  };

  const subscales = order.map((key) => {
    const score = scoreMap[key];
    const band: DasBand = score >= SUBSCALE_NORMAL_MIN && score <= SUBSCALE_NORMAL_MAX
      ? "normal"
      : "dysfunctional";
    type SubResult = { key: DasSubscaleKey; score: number; band: DasBand; direction?: "externalControl" | "autonomy" };
    const result: SubResult = { key, score, band };
  ...
  return {
    report: { subscales, totalScore: row.total_score, depressionProne: row.depression_prone },
    respondent: {
      firstName: row.first_name,
      email: row.email,
      lang: row.lang,
...
function buildSatPayload(row: SatDbRow) {
  ...
  const factorScoreMap: Record<FactorName, number> = {
    Performance:   row.score_performance,
    Affiliation:   row.score_affiliation,
    Aggression:    row.score_aggression,
    Defensiveness: row.score_defensiveness,
    Consciousness: row.score_consciousness,
    Dominance:     row.score_dominance,
    Exhibition:    row.score_exhibition,
    Autonomy:      row.score_autonomy,
    Caregiving:    row.score_caregiving,
    Order:         row.score_order,
    Helplessness:  row.score_helplessness,
  };

  const factors = FACTOR_NAMES.map((name) => {
    const rawScore = factorScoreMap[name];
    const n = norms[name];
    return {
      factor: name,
      rawScore,
  ...
  const subfactorNames: SubfactorName[] = [
    "Self-confirmation",
    "Rational dominance",
  ...
  const sfScoreMap: Record<SubfactorName, number> = {
    "Self-confirmation":        row.sf_self_confirmation,
    "Rational dominance":       row.sf_rational_dominance,
    "Aggressive nonconformity": row.sf_aggressive_nonconformity,
    "Passive dependence":       row.sf_passive_dependence,
    "Sociability":              row.sf_sociability,
    "Agreeableness":            row.sf_agreeableness,
  };

  const subfactors = subfactorNames.map((name) => {
    const score = sfScoreMap[name];
...
export async function GET(req: NextRequest) {
  ...
  if (type === "acsi") {
    const rows = await dbSelect<AcsiDbRow>("acsi_results", { "id": `eq.${id}` });
    if (!rows.length) return NextResponse.json({ error: "not found" }, { status: 404 });
    return NextResponse.json(buildAcsiPayload(rows[0]));
  }

  if (type === "csai") {
    const rows = await dbSelect<CsaiDbRow>("csai_results", { "id": `eq.${id}` });
    if (!rows.length) return NextResponse.json({ error: "not found" }, { status: 404 });
    return NextResponse.json(buildCsaiPayload(rows[0]));
  }

  if (type === "das") {
    const rows = await dbSelect<DasDbRow>("das_results", { "id": `eq.${id}` });
    if (!rows.length) return NextResponse.json({ error: "not found" }, { status: 404 });
    return NextResponse.json(buildDasPayload(rows[0]));
  }

  if (type === "sat") {
    const rows = await dbSelect<SatDbRow>("sat_results", { "id": `eq.${id}` });
    if (!rows.length) return NextResponse.json({ error: "not found" }, { status: 404 });
    return NextResponse.json(buildSatPayload(rows[0]));
  }
