...
type PrepLift = {
  ...
  title: string | null; video_url: string | null; lift_date: string | null;
...
export async function GET(req: NextRequest) {
  ...
  const rows = await dbSelect<PrepLift>("prep_lifts", {
    ...
    select: "id,athlete_id,lift,weight_kg,title,video_url,lift_date,athlete_notes,coach_notes,created_at",
...
export async function POST(req: NextRequest) {
  const user = await getUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  let body: { lift: string; weight_kg?: number; title?: string; video_url?: string; lift_date?: string; athlete_notes?: string };
...
export async function PATCH(req: NextRequest) {
  const user = await getUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  let body: { id: string; lift?: string; weight_kg?: number; title?: string; video_url?: string; lift_date?: string; athlete_notes?: string; coach_notes?: string };
  try { body = await req.json(); } catch { return NextResponse.json({ error: "Invalid JSON" }, { status: 400 }); }
