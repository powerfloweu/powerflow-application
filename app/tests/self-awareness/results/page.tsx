...
export default function ResultsPage() {
  ...
  React.useEffect(() => {
    ...
    const adminRef = typeof window !== "undefined"
      ...
      : null;
    if (adminRef) {
      fetch(`/api/admin/result-detail?type=sat&id=${encodeURIComponent(adminRef)}`)
    ...
    const coachRef = typeof window !== "undefined"
      ...
      : null;
    if (coachRef) {
      fetch(`/api/coach/result-detail?type=sat&id=${encodeURIComponent(coachRef)}`)
        .then((r) => r.ok ? r.json() : null)
        .then((data) => {
          if (data) {
            setPayload(data);
    ...
    const ownerRef = typeof window !== "undefined"
      ...
      : null;
    if (ownerRef) {
      fetch(`/api/test/result?type=sat&ref=${encodeURIComponent(ownerRef)}`)
        .then((r) => r.ok ? r.json() : null)
        .then((data) => {
          if (data?.report) {
        ...
        if (!alreadyUnlocked) {
          ...
          fetch("/api/athlete/assigned-tests")
            ...
            .then((assignments: { test_slug: string }[]) => {
              if (assignments.some((a) => a.test_slug === "sat")) {
                try { localStorage.setItem(UNLOCK_KEY, "1"); } catch {}
                setUnlocked(true);
              }
            })
