...
import type { AcsiReport, AcsiSubscaleKey, Band } from "../../../../lib/tests/acsi/scoring";
import { ACSI_INTRO, SUBSCALE_INTERPRETATIONS } from "../../../../lib/tests/acsi/interpretations";
import { POPULATION_AVERAGES } from "../../../../lib/tests/acsi/norms";
...
type StoredPayload = { report: AcsiReport; respondent: Respondent };

// ---------------------------------------------------------------------------
// Subscale display order + column name map for DB
// ---------------------------------------------------------------------------

const SUBSCALE_ORDER: AcsiSubscaleKey[] = [
  "coping",
  "peaking",
  "goalSetting",
  "concentration",
...
const ui = {
  en: {
    headerTag: "PowerFlow \u00B7 ACSI-28 Profile",
    printDocTitle: "ACSI-28 Report",
    profileTitle: (name: string) => `${name}'s profile`,
    refNorms: (g: string) => `Reference norms: ${g}`,
    genderMale: "Male",
    genderFemale: "Female",
    chartTitle: "Your coping skills at a glance",
    chartFootnote:
      "Each bar shows your score out of 16. The tick mark indicates the population average for your reference group.",
    ...
    noResultsBody:
      "You haven't completed the ACSI-28 test on this device. Start the test to see your profile.",
    ...
    upsellBody: (email: string) =>
      `The chart above is a snapshot \u2014 the report explains what each of your seven coping skills means for you, how they interact, and where targeted work will yield the biggest return. We'll pre-fill the Stripe checkout with ${email}.`,
  ...
  hu: {
    headerTag: "PowerFlow \u00B7 ACSI-28 Profil",
    printDocTitle: "ACSI-28 Riport",
    profileTitle: (name: string) => `${name} profilja`,
    refNorms: (g: string) => `Referenciag\u00E9rt\u00E9kek: ${g}`,
    genderMale: "F\u00E9rfi",
    genderFemale: "N\u0151",
    chartTitle: "Megk\u00FCzd\u00E9si k\u00E9szs\u00E9geid egy pillant\u00E1ssal",
    chartFootnote:
      "Minden s\u00E1v a pontsz\u00E1modat mutatja 16-b\u00F3l. A jel\u00F6l\u0151 a referenciacsoport popul\u00E1ci\u00F3s \u00E1tlag\u00E1t jelzi.",
    ...
    noResultsBody:
      "M\u00E9g nem t\u00F6lt\u00F6tted ki az ACSI-28 tesztet ezen az eszk\u00F6z\u00F6n. Ind\u00EDtsd el a tesztet a profilod megtekint\u00E9s\u00E9hez.",
    ...
    unlockedBody:
      "A h\u00E9t alsk\u00E1la narrat\u00EDv\u00E1ja most m\u00E1r l\u00E1that\u00F3 fent. T\u00F6ltsd le PDF-k\u00E9nt az oldalr\u00F3l.",
  ...
  de: {
    headerTag: "PowerFlow \u00B7 ACSI-28 Profil",
    printDocTitle: "ACSI-28 Bericht",
    profileTitle: (name: string) => `${name}s Profil`,
    refNorms: (g: string) => `Referenznormen: ${g}`,
    genderMale: "M\u00E4nnlich",
    genderFemale: "Weiblich",
    chartTitle: "Deine Bew\u00E4ltigungsf\u00E4higkeiten auf einen Blick",
    chartFootnote:
      "Jeder Balken zeigt deine Punktzahl von 16. Die Markierung zeigt den Bevölkerungsdurchschnitt deiner Referenzgruppe.",
    ...
    noResultsBody:
      "Du hast den ACSI-28-Test auf diesem Ger\u00E4t noch nicht abgeschlossen. Starte den Test, um dein Profil zu sehen.",
    ...
    upsellBody: (email: string) =>
      `Das obige Diagramm ist eine Momentaufnahme \u2014 der Bericht erkl\u00E4rt, was jede deiner sieben Bew\u00E4ltigungsf\u00E4higkeiten f\u00FCr dich bedeutet, wie sie zusammenwirken und wo gezielte Arbeit den gr\u00F6ssten Nutzen bringt. Wir f\u00FCllen die Stripe-Kasse mit ${email} vor.`,
    upsellCta: "Vollst\u00E4ndigen Bericht freischalten \u00B7 \u20AC9",
    upsellFine:
    ...
    unlockedTitle: "Danke",
    unlockedBody:
      "Alle sieben Subskalenberichte werden jetzt oben angezeigt. Lade eine PDF-Kopie dieser Seite herunter.",
...
export default function AcsiResultsPage() {
  ...
  React.useEffect(() => {
    ...
    const adminRef = typeof window !== "undefined"
      ...
      : null;
    if (adminRef) {
      fetch(`/api/admin/result-detail?type=acsi&id=${encodeURIComponent(adminRef)}`)
    ...
    const ownerRef = typeof window !== "undefined"
      ...
      : null;
    if (ownerRef) {
      fetch(`/api/test/result?type=acsi&ref=${encodeURIComponent(ownerRef)}`)
        .then((r) => r.ok ? r.json() : null)
        .then((data) => {
          if (data?.report) {
    ...
    if (typeof window !== "undefined") {
      ...
      if (sessionId && sessionId.startsWith("cs_")) {
        ...
        if (!alreadyUnlocked) {
          ...
          if (resultRef) {
            fetch(
              `/api/acsi/check-paid?ref=${encodeURIComponent(resultRef)}`,
            )
              .then((r) => r.json())
          ...
          fetch("/api/athlete/assigned-tests")
            ...
            .then((assignments: { test_slug: string }[]) => {
              if (assignments.some((a) => a.test_slug === "acsi")) {
  ...
  const openCheckout = React.useCallback(() => {
    ...
    try {
      resultRef =
        ...
        `pfac_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    } catch {
      resultRef = `pfac_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    }
  ...
  const tParagraphs = (obj: { en: string[]; hu: string[]; de: string[] }): string[] =>
    obj[lang] ?? obj.en;
  const tStr = (obj: { en: string; hu: string; de: string }): string =>
    obj[lang] ?? obj.en;

  // Build bar chart data
  const popAvg = POPULATION_AVERAGES[respondent.gender];
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-purple-500/25 bg-gradient-to-br from-[#1C0F30] via-[#110B1E] to-[#050608] p-8 sm:p-10">
          ...
          <div className="relative mt-7 grid grid-cols-3 gap-4 border-t border-white/5 pt-6">
            <div>
              <p className="font-saira text-[9px] font-semibold uppercase tracking-[0.24em] text-zinc-300">{c.totalScoreLabel}</p>
              <div className="mt-1 flex items-baseline gap-1">
                <span className="font-saira text-3xl font-extrabold tabular-nums text-white">{report.totalScore}</span>
                <span className="font-saira text-sm text-zinc-300">{c.totalScoreMax}</span>
              </div>
            </div>
            <div>
              <p className="font-saira text-[9px] font-semibold uppercase tracking-[0.24em] text-zinc-300">{c.strongest}</p>
              <p className="mt-1 font-saira text-sm font-bold leading-tight text-emerald-300">{highestSub.label}</p>
              <p className="mt-0.5 font-saira text-[10px] text-zinc-300">{highestSub.score}/{highestSub.max}</p>
            </div>
            <div>
              <p className="font-saira text-[9px] font-semibold uppercase tracking-[0.24em] text-zinc-300">{c.focusArea}</p>
              <p className="mt-1 font-saira text-sm font-bold leading-tight text-amber-300">{lowestSub.label}</p>
              <p className="mt-0.5 font-saira text-[10px] text-zinc-300">{lowestSub.score}/{lowestSub.max}</p>
            </div>
          </div>
        </div>
        ...
        {unlocked && (
          <>
            ...
            <div className="mt-12 space-y-3">
              ...
              {tParagraphs(ACSI_INTRO).map((p, i) => (
