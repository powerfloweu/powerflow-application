...
export default function CsaiResultsPage() {
  ...
  React.useEffect(() => {
    ...
    const adminRef = typeof window !== "undefined"
      ...
      : null;
    if (adminRef) {
      fetch(`/api/admin/result-detail?type=csai&id=${encodeURIComponent(adminRef)}`)
    ...
    const coachRef = typeof window !== "undefined"
      ...
      : null;
    if (coachRef) {
      fetch(`/api/coach/result-detail?type=csai&id=${encodeURIComponent(coachRef)}`)
        .then((r) => r.ok ? r.json() : null)
        .then((data) => {
          if (data) {
            setPayload(data);
        ...
        if (!alreadyUnlocked) {
          ...
          fetch("/api/athlete/assigned-tests")
            ...
            .then((assignments: { test_slug: string }[]) => {
              if (assignments.some((a) => a.test_slug === "csai")) {
